        ...
        for entry in data["entry"]:
            for messaging_event in entry["messaging"]:
                if messaging_event.get("message"):  # Checking if the messaging even contains a message field.
                    ...
                    response_text = parse_natural_text(sender_id, message_text)
                    send_message_staggered(sender_id, response_text)  # Sending a response to the user.
...
def parse_natural_text(sender_id, user_text):
    # Set session id
    if sender_id not in session_ids:
        session_ids[sender_id] = str(uuid.uuid4())

    ai.session_id = session_ids[sender_id]
    ...
    if response_status == 200:
        ...
        return "Xin lỗi mình không hiểu ý bạn."
